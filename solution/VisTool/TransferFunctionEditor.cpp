...
{	
    ...
    v2iSizeTfEdt_ = TransferFunctionLine::Vec2i(v2iSizeUI_.x-(2*DX11TFEDITOR_CELL_SPACING), v2iSizeUI_.y-(2*DX11TFEDITOR_CELL_SPACING));
...
void TransferFunctionEditor::updateUiPosition( UINT uiBBWidth, UINT uiBBHeight )
{
	...
	INT32 pos[2] = { max(0, uiBBWidth - iSizeX - iBorder ), max(0, uiBBHeight - dim[1] - iBorder) };
...
HRESULT TransferFunctionEditor::onCreateDevice(ID3D11Device* pd3dDevice)
{
	...
	pTfLineAlpha_	= new TransferFunctionLine( D3DXVECTOR4(0,1,1,1), v2iSizeTfEdt_.x - 1, v2iSizeTfEdt_.y - 1);
	pTfLineR_ 		= new TransferFunctionLine( D3DXVECTOR4(1,0,0,1), v2iSizeTfEdt_.x - 1, v2iSizeTfEdt_.y - 1);
	pTfLineG_ 		= new TransferFunctionLine( D3DXVECTOR4(0,1,0,1), v2iSizeTfEdt_.x - 1, v2iSizeTfEdt_.y - 1);
	pTfLineB_ 		= new TransferFunctionLine( D3DXVECTOR4(0,0,1,1), v2iSizeTfEdt_.x - 1, v2iSizeTfEdt_.y - 1);
	...
	if( it == s_mapEffect_.end() || (*it).second == NULL )
	{
		...
		std::ifstream file("TransferFunctionEditor.fxo", std::ios::in | std::ios::binary);
...
void TransferFunctionEditor::drawTransferFunction( )
{
    ...
    TransferFunctionLine::Vec2i v2iTopLeftCorner(max(v2iTopLeftCorner_.x-DX11TFEDITOR_CELL_SPACING,0), max(v2iTopLeftCorner_.y-DX11TFEDITOR_CELL_SPACING,0));
	D3D11_VIEWPORT SMVP;
...
{	
	...
	for( int ch = 0; ch < 4; ch++)
	{
		...
		{
            D3DXVECTOR2 v(static_cast<float>(pCp->v2iPt_.x)/static_cast<float>(v2iSizeTfEdt_.x-1), static_cast<float>(pCp->v2iPt_.y)/static_cast<float>(v2iSizeTfEdt_.y-1));
			binary_file->write(reinterpret_cast<char *>(&(v)), sizeof(D3DXVECTOR2));
			pCp = pCp->pNext_;
		}
	}
	...
	float fAlphaScale = 1.0f; // alpha scale not used in this version
	binary_file->write(reinterpret_cast<const char*>(&fAlphaScale), sizeof(fAlphaScale));
...
void TransferFunctionEditor::loadTransferFunction(std::ifstream* binary_file)
{
	...
	for( int ch = 0; ch < 4; ch++ )
	{
        ...
        TransferFunctionLine::Vec2i v2iPoint(0,0);
        D3DXVECTOR2 v;

		binary_file->read(reinterpret_cast<char *>(&v),sizeof(D3DXVECTOR2));
	...
	float fAlphaScale; // alpha scale not used in this version
	binary_file->read(reinterpret_cast<char*>(&fAlphaScale), sizeof(fAlphaScale));
...
void TransferFunctionEditor::moveSelectedControlPoint( float fPos, int iDim )
{
    ...
    TransferFunctionLine::Vec2i v2iPosition = paChannels_[ iaCurrChannelsOrder_[0] ]->getControlPoint( iSelectedControlPoint_ );
