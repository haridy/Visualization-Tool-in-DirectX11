...
TransferFunctionLine::TransferFunctionLine(D3DXVECTOR4 v4fLineColor, UINT uiMaxX, UINT uiMaxY) 
	: pVb_(NULL)
	, pVl_(NULL)
	, pFirstPoint_(NULL)
	, uiNumPoints_(0)
...
{
    pData_[index] = D3DXVECTOR2(static_cast<float>(pCp->v2iPt_.x)/static_cast<float>(v2iMaxXY_.x), static_cast<float>(pCp->v2iPt_.y)/static_cast<float>(v2iMaxXY_.y) );
}
...
int TransferFunctionLine::getControlPointId(Vec2i v2iPos)
{
	...
	{
		i++;
        D3DXVECTOR2 v2fDist = D3DXVECTOR2( static_cast<float>(pCurr_point->v2iPt_.x - v2iPos.x), static_cast<float>(pCurr_point->v2iPt_.y - v2iPos.y) );
		float fLen = ceil(D3DXVec2Length(&v2fDist));
...
TransferFunctionLine::Vec2i TransferFunctionLine::getControlPoint(int iCpId)
{
	sControlPoint* pCurr_point = pFirstPoint_;
...
void TransferFunctionLine::moveControlPoint(int iCpId, Vec2i v2iPt)
{
	...
	pCurr_point->v2iPt_.y = min( v2iMaxXY_.y, max(v2iPt.y, 0));
...
HRESULT TransferFunctionLine::createVertexBuffer()
{
    ...
    pData_ = new D3DXVECTOR2[ iNumElements ];

	UINT uiSize = sizeof(D3DXVECTOR2) * iNumElements;
...
{
	UINT  uiSize = sizeof(D3DXVECTOR2) * uiNumPoints_;
...
void TransferFunctionLine::draw( )
{
	...
	UINT uiStride = sizeof(D3DXVECTOR2);
	UINT uiOffset = 0;
